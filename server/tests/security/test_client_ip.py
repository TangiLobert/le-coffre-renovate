...
class TestResolveClientIp:
    def test_given_no_xff_header_when_resolving_should_return_peer_host(self):
        request = _request(peer="10.0.0.5")

        assert resolve_client_ip(request, trusted_proxies={"127.0.0.1"}, hops=1) == "10.0.0.5"

    def test_given_no_client_and_no_xff_when_resolving_should_return_unknown(self):
        request = _request(peer=None)

        assert resolve_client_ip(request, trusted_proxies={"127.0.0.1"}, hops=1) == "unknown"

    def test_given_untrusted_peer_when_resolving_should_ignore_xff_header(self):
    ...
    def test_given_one_trusted_hop_when_resolving_should_use_rightmost_xff_entry(self):
        request = _request(peer="127.0.0.1", xff="203.0.113.1, 10.0.0.5")

        assert resolve_client_ip(request, trusted_proxies={"127.0.0.1"}, hops=1) == "10.0.0.5"

    def test_given_n_trusted_hops_when_resolving_should_use_nth_from_right_xff_entry(self):
    ...
    def test_given_xff_shorter_than_hops_when_resolving_should_fall_back_to_peer(self):
        request = _request(peer="127.0.0.1", xff="10.0.0.5")

        assert resolve_client_ip(request, trusted_proxies={"127.0.0.1"}, hops=3) == "127.0.0.1"

    def test_given_whitespace_in_xff_entries_when_resolving_should_strip_them(self):
        request = _request(peer="127.0.0.1", xff="  203.0.113.1  ,  10.0.0.5  ")

        assert resolve_client_ip(request, trusted_proxies={"127.0.0.1"}, hops=1) == "10.0.0.5"

    def test_given_empty_xff_header_when_resolving_should_return_peer(self):
    ...
    def test_given_non_positive_hops_when_resolving_should_return_peer(self, hops: int):
        request = _request(peer="127.0.0.1", xff="10.0.0.5")
