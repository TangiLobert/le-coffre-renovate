...
def test_should_update_password(use_case: UpdatePasswordUseCase, password_repository: InMemoryPasswordRepository):
    # Arrange
    original_password = Password(id=UUID("7d742e0e-bb76-4728-83ef-8d546d7c62e5"), name="original", encrypted_value="encrypted(original)", folder="folder")
    password_repository.save(original_password)
    updated_password = CreatePasswordCommand(id=original_password.id, name="updated", decrypted_password="updated", folder="folder")
...
def test_should_raise_exception_when_password_not_found(use_case: UpdatePasswordUseCase):
    non_existent_password_id = UUID("7d742e0e-bb76-4728-83ef-8d546d7c62e5")
    password_data = CreatePasswordCommand(id=non_existent_password_id, name="original", decrypted_password="encrypted(original)", folder="folder")
