...
def test_given_owner_when_unsharing_from_user_with_read_access_should_revoke_access(
    ...
    use_case.execute(UnshareResourceCommand(owner_id, user_id, password.id))
...
def test_given_non_owner_when_unsharing_should_fail(
    ...
    with pytest.raises(PasswordAccessDeniedError):
        use_case.execute(UnshareResourceCommand(not_owner_id, user_id, password.id))
...
def test_given_owner_when_unsharing_from_another_owner_should_fail(
    ...
    password_repository.save(password)
    password_permissions_repository.set_owner(first_owner_id, password.id)
    password_permissions_repository.set_owner(second_owner_id, password.id)
    password_permissions_repository.grant_access(
        user_id, password.id, PasswordPermission.READ
    )
    ...
    with pytest.raises(CannotUnshareWithOwnerError):
        use_case.execute(
            UnshareResourceCommand(first_owner_id, second_owner_id, password.id)
        )
