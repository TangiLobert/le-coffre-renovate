...
import { generatePassword, estimatePasswordStrength, type PasswordGeneratorOptions } from '@/utils/passwordGenerator';
...
<template>
  <div class="password-generator">
    <Button label="Generate Password" icon="pi pi-bolt" @click="showGenerator = !showGenerator" severity="secondary"
      outlined size="small" class="w-full" />

    <div v-if="showGenerator" class="mt-4 p-4 border surface-border rounded-lg space-y-4 surface-ground">
      <!-- Generated Password Display -->
      <div class="flex flex-col gap-2">
        <label class="font-semibold text-sm">Generated Password</label>
        <div class="flex gap-2">
          <InputText v-model="generatedPassword" readonly class="flex-1 font-mono"
            :class="{ 'opacity-50': !generatedPassword }" />
          <Button icon="pi pi-refresh" @click="generate" severity="secondary" outlined v-tooltip.top="'Regenerate'" />
          <Button icon="pi pi-copy" @click="copyToClipboard" severity="secondary" outlined
        ...
        <div v-if="passwordStrength" class="flex items-center gap-2">
          <div class="flex-1 h-2 surface-200 rounded-full overflow-hidden">
            <div class="h-full transition-all duration-300" :class="strengthColor"
              :style="{ width: `${(passwordStrength.entropy / 128) * 100}%` }"></div>
          </div>
          <span class="text-sm font-medium">{{ strengthLabel }}</span>
          <span class="text-xs text-muted-color">{{ Math.round(passwordStrength.entropy) }} bits</span>
        </div>
      </div>
      ...
      <div class="flex gap-2 pt-2">
        <Button label="Use This Password" @click="usePassword" icon="pi pi-check"
          :disabled="!generatedPassword || !isValidOptions" class="flex-1" />
      </div>
    </div>
  </div>
</template>
