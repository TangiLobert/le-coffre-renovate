  ...
  <div class="surface-ground rounded-lg p-4 hover:surface-hover transition-colors">
    <div class="flex justify-between items-start">
      <div class="flex-1">
        ...
        <div v-if="detailFetched && (loginValue || urlValue)" class="flex items-center gap-4 mb-2 text-sm">
          <span v-if="loginValue" class="flex items-center gap-1 text-color-secondary">
            <i class="pi pi-user text-xs" />
            {{ loginValue }}
          </span>
          <a v-if="urlValue" :href="urlValue" target="_blank" rel="noopener noreferrer"
